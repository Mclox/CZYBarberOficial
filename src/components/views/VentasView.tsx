...
import {
  Receipt, Plus, Search, Eye, FileDown, ShoppingCart, TrendingUp,
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Briefcase,
  Scissors, Package, CreditCard, Trash2, AlertCircle, RotateCcw, CheckCircle2
} from 'lucide-react';
import { toast } from 'sonner';
import { useAuth } from '../../features/auth';
import { exportToExcelXLSX } from '../../shared/lib/exportUtils';
      ...
      <Sheet open={dialogOpen} onOpenChange={setDialogOpen}>
        <SheetContent side="right" className="sm:max-w-2xl w-full flex flex-col p-0 h-full">
          <SheetHeader className="p-6 border-b bg-background flex-none">
            ...
            <Alert className="bg-blue-50 border-blue-200 mt-4">
              <AlertCircle className="h-4 w-4 text-blue-600" />
              <AlertDescription className="text-blue-700 text-xs">Ventas registradas únicamente desde el establecimiento físico.</AlertDescription>
            </Alert>
          </SheetHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {/* Solución al error: Usamos "0" en lugar de "" */}
                      <SelectItem value="0">Cliente General</SelectItem>
                      {clientes.map(c => (
                ...
                <Tabs value={activeTab} onValueChange={(v: any) => { setActiveTab(v); setItemSeleccionado(''); }} className="w-full">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Input type="number" min="1" value={cantidadItem} onChange={e => setCantidadItem(e.target.value)} className="h-11 text-center font-bold" />
                        <Button type="button" onClick={handleAgregarItem} className={`h-11 px-4 ${activeTab === 'productos' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-orange-600 hover:bg-orange-700'}`}><Plus className="w-5 h-5" /></Button>
                      </div>
                    </div>
                  </div>
                </Tabs>
                ...
                <>
                          ...
                          <TableBody>
                            {detallesVenta.filter((d: any) => d.tipo === 'Servicio').map((detalle: any, index: number) => (
                              <TableRow key={index}>
                                ...
                                <TableCell>{detalle.cantidad}</TableCell>
                                <TableCell>${detalle.precio_unitario_neto.toFixed(2)}</TableCell>
                                <TableCell className="text-right font-bold text-orange-700">${detalle.subtotal_item.toFixed(2)}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                          ...
                          <TableBody>
                            {detallesVenta.filter((d: any) => d.tipo === 'Producto').map((detalle: any, index: number) => (
                              <TableRow key={index}>
                                ...
                                <TableCell>{detalle.cantidad}</TableCell>
                                <TableCell>${detalle.precio_unitario_neto.toFixed(2)}</TableCell>
                                <TableCell className="text-right font-bold text-blue-700">${detalle.subtotal_item.toFixed(2)}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                  ...
                  <div className="rounded-xl border-2 border-[#D4AF37]/30 bg-gradient-to-br from-[#D4AF37]/5 to-[#B8941F]/5 overflow-hidden">
                    <div className="p-4 space-y-2">
                      {detallesVenta.some((d: any) => d.tipo === 'Servicio') && (
                        <div className="flex justify-between items-center text-sm">
                          <span className="flex items-center gap-2 text-orange-600 font-medium"><Scissors className="w-3.5 h-3.5" /> Servicios</span>
                          <span className="font-bold text-orange-700">${detallesVenta.filter((d: any) => d.tipo === 'Servicio').reduce((s: number, d: any) => s + d.subtotal_item, 0).toFixed(2)}</span>
                        </div>
                      )}
                      {detallesVenta.some((d: any) => d.tipo === 'Producto') && (
                        <div className="flex justify-between items-center text-sm">
                          <span className="flex items-center gap-2 text-blue-600 font-medium"><Package className="w-3.5 h-3.5" /> Productos</span>
